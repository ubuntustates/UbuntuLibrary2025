...
export default function ReaderPage() {
  ...
  const current = EnglishLiteratureSSS2chapters[chapterNumber - 1];
  if (!current) {
  ...
  const prevChapter =
    chapterNumber > 1
      ? `${EnglishLiteratureSSS2basePath}/${chapterNumber - 1}`
      : null;
  const nextChapter =
    chapterNumber < EnglishLiteratureSSS2chapters.length
      ? `${EnglishLiteratureSSS2basePath}/${chapterNumber + 1}`
      : null;
