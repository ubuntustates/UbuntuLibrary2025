...
export default function ReaderPage() {
  ...
  const current = AgriculturalScienceSSS1chapters[chapterNumber - 1];
  if (!current) {
  ...
  const prevChapter =
    chapterNumber > 1
      ? `${AgriculturalScienceSSS1basePath}/${chapterNumber - 1}`
      : null;
  const nextChapter =
    chapterNumber < AgriculturalScienceSSS1chapters.length
      ? `${AgriculturalScienceSSS1basePath}/${chapterNumber + 1}`
      : null;
